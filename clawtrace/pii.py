...
def _review_batch_with_claude(session_id: str, work_items: list[tuple[str, int, str, str]], *, rubric: str | None, timeout_seconds: int = 180) -> list[PIIFinding]:
    """Review a batch of text chunks via a single Claude Code CLI call."""
    command = require_backend_command("claude")
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            proc = subprocess.run(cmd, input=_BATCH_TASK_PROMPT, cwd=tmp,
                                  capture_output=True, text=True, check=False, timeout=timeout_seconds)
        except subprocess.TimeoutExpired as e:
            raise RuntimeError(f"Claude PII review timed out for session {session_id}") from e
        if proc.returncode != 0:
            stderr = (proc.stderr or "").strip()
            raise RuntimeError(f"Claude PII review failed for session {session_id}: {stderr or 'unknown error'}")
        return _read_batch_findings(tmp_path, session_id, "claude", proc.stdout)


def _review_batch_with_codex(session_id: str, work_items: list[tuple[str, int, str, str]], *, rubric: str | None, timeout_seconds: int = 180) -> list[PIIFinding]:
    """Review a batch of text chunks via a single Codex exec call."""
    command = require_backend_command("codex")
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
        _write_batch_inputs(tmp_path, session_id, work_items, rubric)
        ...
        cmd.append(_BATCH_TASK_PROMPT)
        try:
            proc = subprocess.run(cmd, capture_output=True, text=True, timeout=timeout_seconds)
        except subprocess.TimeoutExpired as e:
            raise RuntimeError(f"Codex PII review timed out for session {session_id}") from e
        if proc.returncode != 0:
            stderr = proc.stderr.strip() if proc.stderr else ""
            stdout = proc.stdout.strip() if proc.stdout else ""
            raise RuntimeError(format_codex_runtime_error(proc.returncode, stderr, stdout))
        return _read_batch_findings(tmp_path, session_id, "codex", proc.stdout)


def _review_batch_with_openclaw(session_id: str, work_items: list[tuple[str, int, str, str]], *, rubric: str | None, timeout_seconds: int = 180) -> list[PIIFinding]:
    """Review a batch of text chunks via a single OpenClaw agent call."""
    command = require_backend_command("openclaw")
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
        _write_batch_inputs(tmp_path, session_id, work_items, rubric)
        message = (
        ...
        cmd = [command, "agent", "--message", message, "--local", "--json", "--timeout", str(timeout_seconds)]
        try:
            proc = subprocess.run(cmd, cwd=tmp, capture_output=True, text=True, timeout=timeout_seconds + 10)
        except subprocess.TimeoutExpired as e:
            raise RuntimeError(f"OpenClaw PII review timed out for session {session_id}") from e
        if proc.returncode != 0:
            stderr = (proc.stderr or "").strip()
            raise RuntimeError(f"openclaw PII review failed for session {session_id}: {stderr or 'unknown error'}")
        return _read_batch_findings(tmp_path, session_id, "openclaw", proc.stdout)


_PII_BATCH_RUNNERS = {
    "claude": _review_batch_with_claude,
    "codex": _review_batch_with_codex,
    "openclaw": _review_batch_with_openclaw,
}
...
def review_session_pii_with_claude(session: dict[str, Any], *, ignore_errors: bool = False, rubric: str | None = None, max_workers: int = 4) -> list[PIIFinding]:
    """Backward-compat wrapper — uses Claude backend."""
    return review_session_pii_with_agent(session, backend="claude", ignore_errors=ignore_errors, rubric=rubric, max_workers=max_workers)


def review_session_pii_hybrid(session: dict[str, Any], *, ignore_llm_errors: bool = True, rubric: str | None = None, backend: str = "auto") -> list[PIIFinding]:
    rule_findings = review_session_pii(session)
    agent_findings = review_session_pii_with_agent(session, backend=backend, ignore_errors=ignore_llm_errors, rubric=rubric)
    return merge_findings(rule_findings + agent_findings)
...
def _content_findings_for_text(session_id: str, message_index: int, field: str, text: str) -> list[PIIFinding]:
    ...
    patterns: list[tuple[str, str, str, float, int]] = [
        ...
        # Partial email / identifier with @ (e.g., "kaid.aiagent@" in tabular output)
