"""Tests for agentstracer.parser — JSONL parsing and project discovery."""
...
from agentstracer.parser import (
    _build_project_name,
...
class TestDiscoverProjects:
    def _disable_codex(self, tmp_path, monkeypatch):
        monkeypatch.setattr("agentstracer.parser.PROJECTS_DIR", tmp_path / "no-claude-projects")
        ...
        monkeypatch.setattr("agentstracer.parser.KIMI_SESSIONS_DIR", tmp_path / "no-kimi-sessions")
    ...
    def test_with_projects(self, tmp_path, monkeypatch, mock_anonymizer):
        ...
        monkeypatch.setattr("agentstracer.parser.PROJECTS_DIR", projects_dir)
    ...
    def test_no_projects_dir(self, tmp_path, monkeypatch):
        self._disable_codex(tmp_path, monkeypatch)
        monkeypatch.setattr("agentstracer.parser.PROJECTS_DIR", tmp_path / "nonexistent")
    ...
    def test_parse_nonexistent_project(self, tmp_path, monkeypatch, mock_anonymizer):
        self._disable_codex(tmp_path, monkeypatch)
        monkeypatch.setattr("agentstracer.parser.PROJECTS_DIR", tmp_path / "projects")
        assert parse_project_sessions("nope", mock_anonymizer) == []

    def test_discover_codex_projects(self, tmp_path, monkeypatch):
        self._disable_codex(tmp_path, monkeypatch)
        projects_dir = tmp_path / "projects"
        monkeypatch.setattr("agentstracer.parser.PROJECTS_DIR", projects_dir / "nonexistent")
    ...
    def test_parse_codex_project_sessions(self, tmp_path, monkeypatch, mock_anonymizer):
        monkeypatch.setattr("agentstracer.parser.PROJECTS_DIR", tmp_path / "projects" / "nonexistent")
        monkeypatch.setattr("agentstracer.parser._CODEX_PROJECT_INDEX", {})
    ...
    def test_discover_opencode_projects(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("agentstracer.parser.OPENCODE_DB_PATH", db_path)
        monkeypatch.setattr("agentstracer.parser._OPENCODE_PROJECT_INDEX", {})
...
class TestDiscoverCustomProjects:
    ...
    def test_discover_missing_dir(self, tmp_path, monkeypatch):
        self._disable_others(tmp_path, monkeypatch)
        monkeypatch.setattr("agentstracer.parser.CUSTOM_DIR", tmp_path / "nonexistent")
        projects = discover_projects()
        assert len(projects) == 0
