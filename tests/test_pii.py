...
def test_review_session_pii_with_claude(monkeypatch):
    ...
    monkeypatch.setattr("clawtrace.pii._review_text_with_agent", fake_review)
...
def test_review_session_pii_with_agent_dispatches_backend(monkeypatch):
    ...
    monkeypatch.setattr("clawtrace.pii.resolve_backend", lambda b: "codex")
    monkeypatch.setattr("clawtrace.pii.check_backend_runtime", lambda b: None)
    monkeypatch.setattr("clawtrace.pii._PII_BATCH_RUNNERS", {"codex": fake_runner})
...
def test_review_session_pii_hybrid_merges_rule_and_claude(monkeypatch):
    ...
    monkeypatch.setattr("clawtrace.pii.review_session_pii_with_agent", lambda s, backend="auto", ignore_errors=True, **kw: [{
        "session_id": "s1",
        "message_index": 0,
        "field": "content",
...
def test_review_text_with_agent_unsupported_backend(monkeypatch):
    monkeypatch.setattr("clawtrace.pii.resolve_backend", lambda b: "gemini")
    monkeypatch.setattr("clawtrace.pii.check_backend_runtime", lambda b: None)
...
def test_review_session_pii_with_agent_raises_on_error(monkeypatch):
    ...
    monkeypatch.setattr("clawtrace.pii.resolve_backend", lambda b: "claude")
    monkeypatch.setattr("clawtrace.pii.check_backend_runtime", lambda b: None)

    def failing_runner(*a, **kw):
        raise RuntimeError("backend crashed")

    monkeypatch.setattr("clawtrace.pii._PII_BATCH_RUNNERS", {"claude": failing_runner})
